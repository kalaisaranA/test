...
class PurchaseOrder(BuyingController):
	...
	def update_received_items(self):
		'''
		update bin reserved_qty_for_production
		called from Stock Entry for production, after submit, cancel
		'''
		if self.docstatus==1:
			# calculate required_qty based on submitted stock entries
			self.update_required_qty_for_supplied_items()
	
	def update_required_qty_for_supplied_items(self):
		'''update required qty from submitted stock entries for that item against
			the purchase order'''

		for d in self.supplied_items:
			t_qty = frappe.db.sql('''select sum(qty)
			...
			d.db_set('t_qty', flt(t_qty), update_modified = False)
